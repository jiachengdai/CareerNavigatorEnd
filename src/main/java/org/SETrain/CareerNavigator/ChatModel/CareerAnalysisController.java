...
public class CareerAnalysisController {
  ...
  public Flux<String> analyzeUserProfile(
  ...
  """, resumeSummaryService.getResumeSummary(username));
      ...
      throw new RuntimeException("用户画像分析失败: " + e.getMessage(), e);
    }
  }

  @GetMapping("/analysis/{username}")
  public Flux<String> analyzeCareer(
          @PathVariable String username,
          @RequestParam(required = false) String chatId) {
    try {
      ...
      throw new RuntimeException("职业分析失败: " + e.getMessage(), e);
    }
  }
}
